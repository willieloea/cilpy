...
def _uniform_distribution(local_attractor: float,
                          current_pos: float,
                          mbest_pos: float,
                          alpha: float) -> float:
...
def _gaussian_distribution(local_attractor: float,
                           current_pos: float,
                           mbest_pos: float,
                           alpha: float) -> float:
...
class QPSOSolver(Solver[List[float]]):
    ...
    def step(self) -> None:
        ...
        alpha = self.alpha_start - (self.iteration / self.max_iterations) * (self.alpha_start - self.alpha_end)
