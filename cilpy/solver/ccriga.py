...
def _tournament_selection(population: List[List[float]],
...
def _blend_crossover(parent1: List[float],
                     parent2: List[float],
...
def _uniform_mutation(individual: List[float],
                      bounds: List[Tuple[float, float]],
                      p_mutation: float) -> List[float]:
...
class CCRIGA(Solver[List[float]]):
    ...
    def __init__(self,
        ...
        # --- GA Parameters ---
        if population_size_x % 2 != 0: population_size_x += 1
        if population_size_l % 2 != 0: population_size_l += 1
    ...
    def _initialize_populations(self):
        ...
        self.population_x = [self.problem.initialize_solution() for _ in range(self.pop_size_x)]
        ...
        if self.lambda_dim > 0:
            self.population_l = [self._initialize_lambda_individual() for _ in range(self.pop_size_l)]
        ...
        self.fitnesses_x = [self._calculate_lagrangian(ind, self.gbest_position_l) for ind in self.population_x]
        ...
        if self.lambda_dim > 0:
            self.fitnesses_l = [self._calculate_lagrangian(self.gbest_position_x, ind) for ind in self.population_l]
    ...
    def _calculate_lagrangian(self, x: List[float], lag_mult: List[float]) -> float:
        ...
        if not self.lambda_dim: return obj_val

        num_ineq = len(self.inequality_constraints)
        mu, lam = lag_mult[:num_ineq], lag_mult[num_ineq:]

        ineq_penalty = sum(m * max(0, const(x)) for m, const in zip(mu, self.inequality_constraints))
        eq_penalty = sum(l * abs(const(x)) for l, const in zip(lam, self.equality_constraints))
        return obj_val + ineq_penalty + eq_penalty

    def _clamp_position(self, position: List[float], bounds: List[Tuple[float, float]]) -> List[float]:
    ...
    def _handle_dynamic_change(self):
        ...
        current_best_idx_x = min(range(self.pop_size_x), key=lambda i: self.fitnesses_x[i])
        ...
        if self.lambda_dim > 0:
            current_best_idx_l = max(range(self.pop_size_l), key=lambda i: self.fitnesses_l[i])
    ...
    def step(self) -> None:
        ...
        for _ in range(self.pop_size_x // 2):
            ...
            c1, c2 = (_blend_crossover(p1, p2) if random.random() < self.p_crossover else (p1[:], p2[:]))
            c1 = self._clamp_position(_uniform_mutation(c1, self.bounds_x, self.p_mutation), self.bounds_x)
            c2 = self._clamp_position(_uniform_mutation(c2, self.bounds_x, self.p_mutation), self.bounds_x)
            offspring_pop_x.extend([c1, c2])

        offspring_fit_x = [self._calculate_lagrangian(ind, self.gbest_position_l) for ind in offspring_pop_x]
        
        combined_pop_x = self.population_x + offspring_pop_x
        combined_fit_x = self.fitnesses_x + offspring_fit_x
        sorted_indices = sorted(range(len(combined_fit_x)), key=lambda k: combined_fit_x[k])
        
        self.population_x = [combined_pop_x[i] for i in sorted_indices[:self.pop_size_x]]
        self.fitnesses_x = [combined_fit_x[i] for i in sorted_indices[:self.pop_size_x]]
        ...
        if self.lambda_dim > 0:
            ...
            for _ in range(self.pop_size_l // 2):
                ...
                c1, c2 = (_blend_crossover(p1, p2) if random.random() < self.p_crossover else (p1[:], p2[:]))
                c1 = self._clamp_position(_uniform_mutation(c1, self.bounds_l, self.p_mutation), self.bounds_l)
                c2 = self._clamp_position(_uniform_mutation(c2, self.bounds_l, self.p_mutation), self.bounds_l)
                offspring_pop_l.extend([c1, c2])

            offspring_fit_l = [self._calculate_lagrangian(self.gbest_position_x, ind) for ind in offspring_pop_l]

            combined_pop_l = self.population_l + offspring_pop_l
            combined_fit_l = self.fitnesses_l + offspring_fit_l
            sorted_indices = sorted(range(len(combined_fit_l)), key=lambda k: combined_fit_l[k], reverse=True) # Maximize

            self.population_l = [combined_pop_l[i] for i in sorted_indices[:self.pop_size_l]]
            self.fitnesses_l = [combined_fit_l[i] for i in sorted_indices[:self.pop_size_l]]
