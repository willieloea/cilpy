...
class RIGASolver(Solver[List[float]]):
    ...
    def step(self) -> None:
        ...
        if self.is_dynamic or self.is_constrained_dynamic:
            ...
            current_best_idx = min(range(self.population_size), key=lambda i: self.fitnesses[i])
        ...
        for _ in range(self.population_size // 2):
            # Selection
            parent1 = _tournament_selection(self.population, self.fitnesses, self.tournament_size)
            parent2 = _tournament_selection(self.population, self.fitnesses, self.tournament_size)
        ...
        sorted_indices = sorted(range(len(combined_fitnesses)), key=lambda k: combined_fitnesses[k])
        
        next_population = [combined_population[i] for i in sorted_indices[:self.population_size]]
        next_fitnesses = [combined_fitnesses[i] for i in sorted_indices[:self.population_size]]
        ...
        if self.num_immigrants > 0:
            ...
            worst_indices = range(self.population_size - self.num_immigrants, self.population_size)
